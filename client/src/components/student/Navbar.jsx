...
const Navbar = () => {
    ...
    return (
        <div className={`flex items-center justify-between px-4 sm:px-10 md:px-14 lg:px-36 border-b border-gray-500 py-4  ${isBookListPage ? 'bg-white' : 'bg-cyan-100/70'}`}>
            <img onClick={()=>navigate('/')} src={assets.SajhaKitabLogo} alt="Logo" className='w-28 lg:w-32 cursor-pointer' />

            <div className='hidden md:flex items-center gap-5 text-gray-500'>
                <div className='flex items-center gap-5'>
                    <Button onClick={()=>{navigate('/librarian')}} type="text" className='text-gray-500 p-0 h-auto'>
                        {isLibrarian ? 'Librarian Dashborad': 'Become Librarian'}
                    </Button>
